  ...
  return `
        <section class="admin-card admin-card--panorama">
          ...
          <p class="admin-cover-meta" style="margin:0 0 12px;">
            详情页「360°」缩略图点开后的环顾图。请上传 <strong>2:1</strong> 等距柱状全景（Insta360 导出的 JPG 等，最大 48MB）。<strong>由服务器自动压缩</strong>为最长边 4096px 的 JPEG 后存入 R2。
          </p>
...
function bindEditor() {
  ...
  if (
    ...
    document.getElementById("panoramaUpload")?.addEventListener("change", async (event) => {
      ...
      const file = files[0];
      try {
        showToast("正在上传，服务器将自动压缩…", "success", 12000);
        const form = new FormData();
        form.append("file", file);
